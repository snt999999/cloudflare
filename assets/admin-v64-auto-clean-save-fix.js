/* СОЛНЦАНЕТ v64: Авто-поля + чистое сохранение
   Что делает:
...
(function () {
  ...
  const VERSION = "v64-auto-clean-save-fix";
  ...
  function installFetchPatch() {
    if (window.__SOLNCANET_V64_FETCH_PATCHED__) return;
    window.__SOLNCANET_V64_FETCH_PATCHED__ = true;
  ...
  function install() {
    ...
    installFetchPatch();
    run();
    window.SOLNCANET_V64 = window.SOLNCANET_V64 || {};
    window.SOLNCANET_V64.autoCleanSaveFix = { version: VERSION, apply: applyAutoLayout, cleanAutoFields };
