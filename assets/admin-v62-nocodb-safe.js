/* СОЛНЦАНЕТ v62: безопасный клиент Cloudflare Functions -> NocoDB */
(function () {
  "use strict";

  const VERSION = "v62-nocodb-safe";
  ...
  function message(text, isError) {
    ...
    if (text) console[isError ? "error" : "info"]("СОЛНЦАНЕТ v62:", text);
  }
  ...
  function setButtonBusy(button, busy, busyText) {
    if (!button) return;
    if (busy) {
      button.dataset.v62OldText = button.textContent;
      button.disabled = true;
      button.textContent = busyText || "Сохраняю...";
    } else {
      button.disabled = false;
      if (button.dataset.v62OldText) button.textContent = button.dataset.v62OldText;
      delete button.dataset.v62OldText;
    }
  }

  window.SOLNCANET_V62 = window.SOLNCANET_V62 || {};
  Object.assign(window.SOLNCANET_V62, {
    version: VERSION,
