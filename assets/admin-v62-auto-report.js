/* СОЛНЦАНЕТ v62: авто-отчет с суммами и расчетом ЗП */
(function () {
  "use strict";

  const VERSION = "v62-auto-report";
  ...
  async function loadRecordsFromServer() {
    if (!window.SOLNCANET_V62?.nocodb?.list) throw new Error("Не подключен admin-v62-nocodb-safe.js");
    const payload = await window.SOLNCANET_V62.nocodb.list({ limit: 1000 });
    return window.SOLNCANET_V62.normalize.list(payload);
  }
  ...
  function download(records, options) {
    ...
    const filename = `solncanet_auto_zp_v62_${new Date().toISOString().slice(0, 10)}.csv`;
    downloadCsv(filename, [header, ...body, ...footer]);

    if (window.SOLNCANET_V62?.ui?.message) {
      window.SOLNCANET_V62.ui.message(`Авто-отчет v62 скачан. Строк: ${rows.length}. Сумма: ${totalAmount} ₽. ЗП: ${totalPayroll} ₽.`, false);
    }
  ...
  function installButton() {
    ...
    if (!target || document.getElementById("downloadAutoReportV62Btn")) return;

    const button = document.createElement("button");
    button.id = "downloadAutoReportV62Btn";
    button.type = "button";
    button.textContent = "Авто-отчет ЗП v62";
    ...
    button.addEventListener("click", async () => {
      try {
        ...
        if (window.SOLNCANET_V62?.ui?.message) window.SOLNCANET_V62.ui.message(error.message, true);
  ...
  window.SOLNCANET_V62 = window.SOLNCANET_V62 || {};
  window.SOLNCANET_V62.autoReport = {
    version: VERSION,
