...
export async function onRequest(context) {
  ...
  if (request.method !== "POST") return json({ ok:false, error:"Only POST is allowed" }, 405);

  const token = env.NOCODB_TOKEN;
  const endpoint = env.NOCODB_ENDPOINT || DEFAULT_NOCODB_ENDPOINT;
  if (!token) return json({ ok:false, error:"NOCODB_TOKEN is missing" }, 500);

  let incoming;
  try { incoming = await request.json(); } catch(error) {
...
function getServiceName(payload) { return (payload.eventType && payload.eventType.title) || payload.eventTitle || payload.title || "Онлайн-запись Cal.com"; }
function findAnswer(obj, names) {
  ...
  for (const key of Object.keys(obj)) {
    ...
    const labelNorm = raw && typeof raw === "object" ? norm(raw.label || raw.question || raw.name || raw.title || "") : "";
    for (const name of wanted) {
      if (keyNorm.indexOf(name) !== -1 || labelNorm.indexOf(name) !== -1) return unwrap(raw);
    }
  }
  return "";
}
function unwrap(value) {
  ...
  if (Object.prototype.hasOwnProperty.call(value, "value")) return Array.isArray(value.value) ? value.value.join(", ") : value.value;
...
function timeYekaterinburg(date) { return new Intl.DateTimeFormat("ru-RU", { timeZone:"Asia/Yekaterinburg", hour:"2-digit", minute:"2-digit", hour12:false }).format(date); }
